...
const peerToPresenceKey = new WeakMap<Peer, string>()
const peerToDeviceLabel = new WeakMap<Peer, DeviceLabel>()
...
// 主动给端点推送匹配的任务列表(单次)
export function pushListToPeer(peer: Peer, walletId: string, deviceLabel: DeviceLabel) {
  ...
  try {
    ;(peer as unknown as { send: (text: string) => void }).send(payload)
  } catch (e) {
...
export function walletIdOfPeer(peer: Peer): string | undefined {
  return peerToPresenceKey.get(peer)
}

export function deviceLabelOfPeer(peer: Peer): DeviceLabel | undefined {
