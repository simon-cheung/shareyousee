...
import { calcMD5, formatTime, isModernFileAPIAvailable, doDownloadFromBlob } from '@/utils'
import { fileMapWithoutRoot, dealFilesFromHandler } from '@/utils/files'
...
export const useRecipientTransferStore = defineStore('recipientTransfer', () => {
  ...
  async function doReceive() {
    ...
    try {
          ...
          saveFileFH = await window.showSaveFilePicker({
            ...
            ...(ext && {
              types: [
                {
                  description: '',
                  accept: { 'application/x-fastsend': [`.${ext}` as `.${string}`] }
                }
              ]
            })
          })
      ...
      // 乐观更新:本地立刻把这条 code 从 pendingList 移除,避免服务端 consumePush
      // 没送达(对方可能不在线)时 pushList 又把这条复活
