...
async function importEccKeyFromIkm(ikm: Uint8Array): Promise<CryptoKeyPair> {
  ...
  try {
    // HKDF 派生 32 字节私钥标量
    const hkdfKey = await subtle.importKey('raw', ikm as BufferSource, { name: 'HKDF' }, false, [
      'deriveBits'
    ])
  ...
  try {
    ...
    return await new Promise<CryptoKey | null>((resolve, reject) => {
      ...
      req.onsuccess = () => {
        ...
        db.close()
        // 详细诊断日志:把读出来的字段全部打印,方便排查 structuredClone 后字段缺失
        if (result && typeof result === 'object') {
          const k = result as Record<string, unknown>
          const algo = k['algorithm'] as { name?: string } | undefined
          console.log(
            '[wallet] loadPrivateKey: raw fields type=%s algo=%s extractable=%s usages=%s isCryptoKey=%s',
            k['type'] ?? '(missing)',
            algo?.name ?? '(missing)',
            k['extractable'] ?? '(missing)',
            JSON.stringify(k['usages'] ?? '(missing)'),
            typeof CryptoKey !== 'undefined' && k instanceof CryptoKey
          )
        } else {
          console.log('[wallet] loadPrivateKey: result is null/undefined or not an object')
        }
        if (isValidCryptoKey(result)) {
          console.log('[wallet] loadPrivateKey: PASS validation, returning key')
          resolve(result)
        } else {
          console.warn(
            '[wallet] loadPrivateKey: FAIL validation, returning null (private key NOT deleted)'
          )
          resolve(null)
        }
      }
    ...
    console.warn('loadPrivateKey failed (will not reset DB)', e)
    return null
  }
}

// 用宽松判定:CryptoKey 在 structuredClone 后可能在不同浏览器里字段缺失,
// 但 algorithm.name 一定是 'ECDSA'(我们只存这一种私钥)。
function isValidCryptoKey(v: unknown): v is CryptoKey {
  if (!v || typeof v !== 'object') return false
  const k = v as Record<string, unknown>
  // 直接是 CryptoKey 实例(部分浏览器)
  if (typeof CryptoKey !== 'undefined' && k instanceof CryptoKey) return true
  // structuredClone 后的形态:有 algorithm / extractable / usages 三个核心字段
  const algo = k['algorithm'] as { name?: string } | undefined
  return (
    algo?.name === 'ECDSA' &&
